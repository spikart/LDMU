...
if ($style == '') {
	...
	if ($module->position == 'top-d') $style = 'box';
...
// force module style
if (in_array($module->position, array('headerbar','supersize', 'breadcrumbs', 'logo', 'banner', 'search', 'debug', 'supersize', 'map', 'mosaic'))) {
	$style = 'raw';
	$showtitle = 0;
}
if (in_array($module->position, array('toolbar-r' ,'toolbar-l'))) {
	$style = '';
	$showtitle = 0;
}
...
if ($module->menu) {
	...
	if (isset($params['menu'])) {
		...
		$renderer = 'dropdown';
	} else if (in_array($module->position, array('toolbar-l', 'toolbar-r', 'footer-a'))) {
		$renderer = 'default';
	} else {
		$renderer = 'accordion';
	}
