...
$css[] = sprintf('.jp-main-outer { max-width: %dpx; }', $this['config']->get('template_width'));

// if responsive = off >> width only and not max-width
if ($this['config']->get('responsive') == 0)
$css[] = sprintf('.wrapper, #mainsite, .header-outer, .footer-outer, #headerbar, #supersize, .menu-outer { min-width: %dpx; }', $this['config']->get('template_width'));

// Body Font Size
$css[] = sprintf('body { font-size: %dpx; }', $this['config']->get('body-font-size'));

// Menu First level Font Size
$css[] = sprintf('.menu-dropdown a.level1,.menu-dropdown span.level1 { font-size: %dpx; }', $this['config']->get('menu-font-size'));

// Logo Height
$css[] = sprintf('#logo{ height: %dpx;}', $this['config']->get('logo-height'));

// Menu Margin from TOP
$menu_margin = $this['config']->get('logo-height') -40;
$css[] = sprintf('#menu { margin-top: %dpx; }', $menu_margin *0.5);
...
// load css
$this['asset']->addFile('css', 'css:global.css');
...
$this['asset']->addFile('css', 'template:fonts/font-awesome.css');
...
$fonts = array(
	...
	'quicksand' => 'template:fonts/quicksand.css');
